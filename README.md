...
- an optional responsive Home Assistant navbar with direct links and grouped pop-up menus
...
navbar_items:
  - "home|Home|/"
  - "camera|Cameras|/dashboard-camera/0"
navbar_subitems: []
```
...
Enable `navbar_enabled` to show a floating navbar over the bottom of Protect. A direct button uses
`icon|label|HA path`, for example `home|Home|/lovelace/home`. To create a pop-up group, leave its
path empty (`cog|System|`) and add submenu entries using
`group label|icon|label|HA path`, for example
`System|settings|Settings|/config/dashboard`. Links open in the main Home Assistant window, and the
navbar remains visible while navigating inside Protect. See the app documentation for more
examples.
